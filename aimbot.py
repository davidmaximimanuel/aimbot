...
if LOGTO_ENDPOINT and LOGTO_CLIENT_ID:
...
Just talk to me naturally — I'll understand! 🇳🇬✨"""
        ...
        if not LOGTO_ENDPOINT or not LOGTO_CLIENT_ID:
            await send_text_chunks(chat_id, "⚠️ Web login is not configured yet. Check back soon!", reply_to=message_id)
            return True
        profile = await get_user_profile_data(user_id)
        existing_logto_id = profile.get("logto_id")
        if existing_logto_id:
            empire_id = profile.get("empire_id","N/A")
            await send_text_chunks(chat_id, f"✅ <b>Already linked!</b>\n\n🆔 Empire ID: <b>{empire_id}</b>\n🔗 Logto ID: <code>{existing_logto_id}</code>", reply_to=message_id)
            return True
        state    = _create_oauth_state(user_id, chat_id)
        auth_url = build_logto_auth_url(state)
        await send_text_chunks(chat_id, f"🔗 <b>Link your account to Empire AI Web</b>\n\n<a href=\"{auth_url}\">👉 Sign in / Create Account</a>\n\n⏳ Expires in <b>10 minutes</b>.", reply_to=message_id)
        return True
