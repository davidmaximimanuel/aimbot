...
def check_tasks_background():
    ...
    while True:
        ...
        try:
            now_utc = datetime.now(timezone.utc).isoformat()
            res = (supabase.table("user_tasks").select("*").eq("is_active", True).lte("next_run", now_utc).order("next_run", desc=False).execute())
...
async def create_task_in_db(user_id: str, task_data: dict) -> str:
    ...
    try:
        ...
        row = {"user_id": str(user_id), "task_description": task_data.get("description", "Reminder"), "task_type": task_data.get("type", "one_time"), "scheduled_time": task_data.get("scheduled_time"), "recurrence_pattern": task_data.get("recurrence_pattern"), "recurrence_time": task_data.get("recurrence_time"), "recurrence_days": task_data.get("recurrence_days") or [], "task_category": task_data.get("category", "reminder"), "is_active": True, "next_run": next_run.isoformat() if next_run else None}
    ...
    try:
        ...
        for row in recent_rows:
            try:
                ...
                tstr = _fmt_wat(ts)
            except Exception: tstr = "unknown time"
...
async def get_ai_response(user_text: str, user_id: str, chat_type: str, profile: dict = None, session_summary: str = "", recent_history: str = "", older_context: str = "", web_context: str = "", tool_status: str = "", gap_seconds: float = 0.0) -> Optional[str]:
    try:
        ...
        prompt = build_enhanced_prompt(user_text, user_id, profile, session_summary, recent_history, older_context, web_context, tool_status, gap_seconds)
        if USE_DEEPSEEK and deepseek_client:
...
async def handle_inline_query_async(inline_query):
    ...
    result = InlineQueryResultArticle(id=str(uuid.uuid4()), title=f"AIM: {qtext[:30]}", description=(answer_text or "Click to get AIM's answer")[:100], input_message_content=InputTextMessageContent(message_text=f"🤖 <b>AIM says:</b>\n\n{answer_text}\n\n<i>via @askaimbot</i>" if answer_text else f"🤖 Asking AIM: {qtext}\n⏳ Processing...", parse_mode=ParseMode.HTML))
...
async def handle_message_async(update: Update):
    ...
    if update.message.photo:
        ...
        try:
            ...
            if is_logo:
                description = "This is AIM's logo - a beautiful beam of light representing African intelligence!"
